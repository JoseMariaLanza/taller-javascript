...
const solicitarDinero = () => {
    ...
    if (extraer <= total) {
        billetes.forEach(billete => {
            if (extraer > 0) {
                cantidadBilletes = Math.floor(extraer / billete.valor)

                if (cantidadBilletes > billete.cantidad) {
                    papeles = billete.cantidad
                } else {
                    papeles = cantidadBilletes
                }
        ...
        alert('El cajero no tiene dinero suficiente')
    }
}

const mostrarBilletes = () => {
    const contenedorBilletes = document.getElementById("container_billete")

    entregado.forEach(papel => {
        switch (papel.valor) {
            case 50:
                let img50 = null;
